...
;---------------------------FROM THE OBSERVES VIEWPOINT: Determine which particles are occulted by the body------------------------
    ...
    if (viewpoint ne 'Above Ecliptic' and viewpoint ne 'Moon Spot') then begin
      ...
      ; Print the phase angle and distance 
        if Keyword_Set(debug) then begin
          print, body, ' phase angle from ', viewpoint, ':',  !radeg*cspice_vsep( Observer_Body_vector, Sun_Body_vector )
          print, viewpoint, ' distance to ', body, ' center:', norm(Observer_Body_vector), ' km'
        endif
          
      ; Now that we have the Observer to Body and Sun to Body vectors, we can define a new frame for plotting purposes....
      ; 
      ...
      ; *  This means that in Loc-prime, the projection of the sunward direction is always in the first X dimension                     *                                                        
      ; *                                                                                                                               *
      ; *********************************************************************************************************************************
      ...
      ; Find the rotation matrix to transform particle locations from their current J2000 Body-centered frame into the desired loc_prime coordinate frame.
        cspice_twovec, Observer_Body_vector, 3, Sun_Body_vector, 1, alignment_matrix  ;Re-defines -Observer_Body_vector as 'Z' axis
    ...
    ; For space-based observers, we'll also need the instrument's boresight direction w/ respect to planet center
      ...
      ; Get the planetary coordinates of the tangent point of the boresight ray
        ...
        if Keyword_Set(debug) then begin
          ...
          print, 'UVVS Boresight Tangent Planetary Longitude:', lon[0]*!radeg
          print, 'UVVS Boresight Tangent Planetary Latitude: ', lat[0]*!radeg
          ;stop
        endif
      ...
      ; convert this boresight vector from the IAU Mercury frame in to a body-centered J2000 frame
        ...
        TP_J2000 = TRANSPOSE(IAU_Mercury_to_J2000_transform_matrix) # tangent_point_MERC_IAU
      ...
      ; Move this to a MESSENGER origin and determine it's "ra dec" and then x, y
