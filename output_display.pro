...
;---------------------------FROM THE OBSERVER'S VIEWPOINT: Determine which particles are occulted by the body------------------------
  ...
  ; Particles behind the planet cannot emit. Particles in front can emit.
    ...
    for i  = 0, N_particles-1 do begin
      ...
      if strmid(body, 0, 3) eq '100' then continue     ; ignore any chance of particles oculted by small bodies
...
if part eq 2 then begin ;Once the model reaches its last loop, plot the averaged output.
  ...
  ; Make an axis in RA and Dec coordinates
    if viewpoint eq 'Moon Spot' then begin
      ...
      TICLABELS, Body_RA_WRT_obs*!radeg - (x_center/s[1])*(FOV*cos(Body_dec_WRT_obs)/(3600.)), N_ticks+1., (fov*cos(Body_dec_WRT_obs)/(60.*N_ticks)) / 15., RAlabs, /RA, delta = 1
