...
pro output_display, loc, g, atoms_per_packet, Image_type, Model_Image_RV, loop_number, part, reimpact_loc = reimpact_loc, Label_North = Label_North, Label_phase = Label_phase, $
  ...
  if part eq 1 then begin
    ...
    ; Normal Cases: One object viewed from the center of another object
      if (viewpoint ne 'Above Ecliptic' and viewpoint ne 'Moon Spot') then begin
        ...
        ; FIRST, need to move origin from Parent (usually SS Barycenter) to Observer
        ; Get parent - > viewpoint vector
        cspice_spkezr, PARENT_ID, ephemeris_time, 'J2000', 'LT', viewpoint, parent_viewpoint_state, junk_lt
        ...
        OBSERVATORY_cs, observatory, obs_struct ;longitude is in degrees *West*, need to switch to East Longit for J2000 conversion via cspice_georec
        cspice_georec, (360. - obs_struct.longitude) * CSPICE_RPD(), obs_struct.latitude * CSPICE_RPD(), obs_struct.altitude / 1.e3, radii[0], flat, obs_IAU_Earth
        obs_IAU_Earth_state = [obs_IAU_Earth, 0., 0., 0.] ; extend to include velocity
  
        ; Rotate from IAU_Earth to J2000 (incl. velocities)
        cspice_sxform, 'IAU_EARTH', 'J2000', ephemeris_time, Earth_to_J2000_transform_matrix
        obs_J2000_state = transpose(Earth_to_J2000_transform_matrix) # obs_IAU_Earth_state
        
        ; Construct Parent -> Observer state vector and apply
        parent_observer_state     = parent_viewpoint_state - obs_J2000_state
        loc_obs_centered          = loc
        loc_obs_centered[0:2,*]  += rebin(parent_observer_state[0:2], 3, N_particles)
        loc_obs_centered[5:7,*]  += rebin(parent_observer_state[3:5], 3, N_particles)
        
        ; Now, write a Frames Kernel file that defines the LOC_PRIME frame and load it into SPICE
        write_fk_file
        ...
        loc_obs_centered_state_format  = [loc_obs_centered[0:2,*], loc_obs_centered[5:7,*]]
        loc_prime_state_format = TRANSPOSE(alignment_matrix) # loc_obs_centered_state_format
  ...
  ;        Loc_distance_WRT_obs, Loc_RA_WRT_obs, Loc_dec_WRT_obs
      ...
      ; Inspect particle on-sky coordinates
      ...
      ; Velocity axis from -100 to +100 km/s with binsize of 0.1
      v_min   = -100.
      binsize = 0.1 ; km/s
      nbins   = round(200 / binsize)
        ...
        ; Allocate x,y positions to pixels in the 2d Model_Image_R if within the plot window 
          if x ge 0 and x lt Output_Size_In_Pixels[0] and y ge 0 and y lt Output_Size_In_Pixels[1] then begin 
            ...
            ; Output an image in rayleighs
              ...
              bin = floor((loc_prime[7,i] - v_min)/binsize)
              Model_Image_RV[x,y,bin] += loc[4,i] * occulted[i] * atoms_per_packet * g[i] * 1.e-6
              ;(fractional packet content) * (particle is visible?) * (atoms per packet) * (photons/(atom*s)) * (10^-6 for Rayleighs)
          endif
      endfor 
  ...
  if part eq 2 then begin ;Once the model reaches its last loop, plot the averaged output.
    ...
    ; Generate and plot emission line profiles and temperature map
      if (viewpoint eq 'Earth' or viewpoint eq 'Moon Spot') and Measure_Linewidths then begin
        generate_line_profiles, Model_Image_RV, Model_Image_T=Model_Image_T
        
      ; Write temperature image
        cgPS_open, filename = strcompress(directory + Output_title + '_Temperature.eps'), /ENCAPSULATED, /NOMATCH
          !P.font=1
          !p.charsize=1.5
          device, SET_FONT = 'Helvetica Bold', /TT_FONT
          minValue = 0
          maxValue = Ceil(Median(Model_Image_T)*1.5) ; Ceil(Max(Model_Image_T, /NaN))
  
          loadct, 34, /silent
          cgImage, Model_Image_T, Position = [0.075, 0.125, 0.9, 0.90], minvalue=minvalue, maxvalue=maxvalue, AXKEYWORDS=RA_axis_format, /Axes, $
            /KEEP_ASPECT, Title = Title, charsize =1.2, /Window, /save
          cgColorbar, /Vertical, Position = [!x.window[1]+.07, !y.window[0], !x.window[1]+.1, !y.window[1]], Range=[MinValue, MaxValue], $
            Title = strcompress('' + Particle_data.name + ' Temperature (K)'), TLocation='Left', /right, ticklen = 0, charsize=1.5
        cgPS_Close
      endif
      
  ; Draw the outline of the spectrometer aperture
