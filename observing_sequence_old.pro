Pro Observing_Sequence_old, Meteor_impact_UTC = Meteor_impact_UTC, Plume_Temperature = Plume_Temperature, $
...
;;; Round 2
;      Meteor_impact_UTC             = '2011-08-04 01:45:00'       ; time of the impact
;      Plume_Temperature             = '3500K'                     ; temperature of the impact vapour
;      Surface_distribution = 'Point_[0, -45]'                   ; Location of the impactor
;      loop_times                     = 20.                        ; Bear minimum for any reasonable S/N
;      Na_Lofted                     = 2.8e28                      ; seems like a lot
;      Mg_Lofted                     = 4.*Na_lofted                ; seems like a lot
;      Brightness_multiplier_Na      = 0.5
;      Brightness_multiplier_Mg      = 0.5
      
;; Round 2
      Meteor_impact_UTC             = '2011-08-04 02:05:00'       ; time of the impact
      Plume_Temperature             = '3500K'                     ; temperature of the impact vapour
      Surface_distribution = 'Point_[29, 0]'                     ; Location of the impactor
      loop_times                     = 20.                        ; Bear minimum for any reasonable S/N
      Na_Lofted                     = 3.5e27                      ; seems like a lot
      Mg_Lofted                     = 4.*Na_lofted                ; seems like a lot
...
;restore, Directory+'boresight_pixels.sav'
  ...
  Aperture_pixels = dblarr(2, N_elements(Tim_utc_time), 4) ; log the location of the UVVS aperture corners at each time
  for i = 0, N_elements(Tim_utc_time)-1 do begin
...
;        loop_times_this_run = loop_times, Upward_flux_at_exobase_this_run = Mg_Lofted
    ...
    ;Aperture_pixels[*,i,*] = Aperture_Corners
  endfor
...
;  print, data[sorted[0:9]], format = '(A-20, I7, F12.5, F10.5, I9)'
  ...
  mpgFilename = Directory+Write_directory+'\Sequence.mp4'
  framerate   = 10 ; FPS
  ...
  for i  = 0, N_elements(Tim_utc_time) -1 do begin
    ...
    ; intermediate files.
      ...
      ;cgPS_Open, Directory + '\test_geometry.eps', /ENCAPSULATED;, /Quiet
      ...
      ; Log the brightness and Mark the boresight's tanget point  
        ...
        ;frame_Na[ boresight_pixels[0,i]-1:boresight_pixels[0,i]+1, boresight_pixels[1,i]-1:boresight_pixels[1,i]+1 ] = max(test_frame_Na)    ; can delete, using Polyfill and the actual UVVS aperture instead
        ;frame_Mg[ boresight_pixels[0,i]-1:boresight_pixels[0,i]+1, boresight_pixels[1,i]-1:boresight_pixels[1,i]+1 ] = max(test_frame_Mg)   
  
 TO_DO = 'USE THE ACUTAL APERTURE FOR PHOTOMETRY, UNSTEAD OF THE BORESIGHT +/- 1 PIXEL.' ; HACK HACK HACK, edit the above code for photomtry in the uvvs aperture
        ...
        ; Mark the UVVS Aperture projection, the coordinates here need to be in degrees  
          cgPolygon, [transpose(Aperture_pixels[0,i,*]), Aperture_pixels[0,i,0]]*platescale/3600.+range[0], $
                     [transpose(Aperture_pixels[1,i,*]), Aperture_pixels[1,i,0]]*platescale/3600.+range[0], COLOR='White', /fill
        ...
        ; Mark the UVVS Aperture projection, the coordinates here need to be in degrees
          cgPolygon, [transpose(Aperture_pixels[0,i,*]), Aperture_pixels[0,i,0]]*platescale/3600.+range[0], $
            ...
            ; Mark the lat & lon grid  
              ...
              cgcontour, ob.lat, /onimage, levels = lat_contours, LABEL = 0, color = 'green', THICK = .5
              cgcontour, ob.lon, /onimage, levels = lon_contours, LABEL = 1, color = 'green', THICK = .5

            ; Mark the sub-solar longitude and the impactor lat and lon  
              ...
              ;  color = 'red', THICK = 0.5, LABEL = 0, OLEVELS = OLEVELS
              ;cgcontour, ob.lon, /onimage, levels = sub_solar_lon*!radeg, color = 'YELLOW', THICK = 0.5 ; Mark the Sub-Solar Point
              ;hack_levels = [min(ob.lon) + findgen(100)/1000. - .05, max(ob.lon) + findgen(100)/1000. - .05]
              ;cgcontour, ob.lon, /onimage, levels = hack_levels, color = 'green', THICK = 0.5, label = 0 ; Contour finds a line between 0 and 360 longitude, re-draw green over this
 
 
 
        ; mark a countour for the UVVS tangent point Lat & lon
        ;cgcontour, ob.lon, /onimage, levels = 181.85608, color = 'red', THICK = 0.5, label = 0 ; replace with the tangent longitude, verify correct boresight
  ...
  ; Compare the results by correlation
    ...
    save, correl, filename = Directory+ 'Correlation_results\' + write_directory + '_Na_correlation_Hi_Res.sav'
