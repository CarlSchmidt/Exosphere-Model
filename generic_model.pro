pro generic_model, Time_range_this_run = Time_range_this_run, test_particle_this_run = test_particle_this_run, line_this_run = line_this_run, $
                   Output_title_this_run = Output_title_this_run, UTC_this_run = UTC_this_run, Speed_Distribution_this_run = Speed_Distribution_this_run, $
                   Surface_Distribution_this_run = Surface_Distribution_this_run, Loop_times_this_run = Loop_times_this_run, $
                   Upward_flux_at_exobase_this_run = Upward_flux_at_exobase_this_run, restore_aloft_filename = restore_aloft_filename
                   
; Carl Schmidt, BU, 2025
; Originally adapted from Jody Wilson, Boston University, 2007 
;
; MODIFICATIONS:
;    5/7/2020: edits to output display for MESSENGER UVVS viewing comparisons
;
; DEPENDENCIES:
;
...
;Body               = '1003667'                            ; e.g., 'Mercury', 'Moon', use NAIF body ID number for comets e.g. '1000012' = 'CHURYUMOV-GERASIMENKO'
Body               = 'Mercury'                              ; e.g., 'Mercury', 'Moon', use NAIF body ID number for comets e.g. '1000012' = 'CHURYUMOV-GERASIMENKO'
                                            ; 
UTC                = '2013 November 29 14:25'             ; Winslow et al. (2020) ICME passage time
;UTC                = '2020-Nov-13 23:20'                  ; Mercury Peak Na radiation pressure
;UTC                = '2007-Jun-06 12:40'                  ; Mercury Aphelion
;UTC                = '2026 Aug 05 06:49'                ; PLunar Falcon 9 impact
;UTC                = 'Apr 08, 2005 22:10:00'              ; Solar eclipse from McDonald
;UTC                = '2022-Sep-29 09:35:00'                ; Rough time of the Juno Europa Flyby
;UTC                = '2020-Jul-15 03:42'                  ; NEOWISE Jul 8
...
Speed_distribution = 'Sputtering_ST'                       ; Set the speed distribution the particles will be released with
                                                           ...
                                                           ;          'Shematovich'       Shematovich (2013, DOI:10.1134/S0038094613060087), requires an associated input file 
                                                           ;          'Johnson_2002'      Johnson et al. (2002, DOI:10.1006/icar.2001.6763). For Na and K desorption from ice. 
                                                           ;          'Sputtering_ST'     A Sigmund-Thompson sputtered distribution. See Wurz et al. (2010; DOI:10.1016/j.pss.2010.08.003)
if keyword_set(Surface_distribution_this_run) then Surface_distribution = Surface_distribution_this_run else $
Surface_distribution = 'From_Map'                          ; Set the surface distribution the particles will be released with
;Surface_distribution = 'Point_[268.,17.]'                          ; Set the surface distribution the particles will be released with
;Surface_distribution = 'Dayside'                          ; Set the surface distribution the particles will be released with
                                                           ...
                                                           ;          'From_Map:'         Use a lat lon map of [360,180] in cm^-2 s-1 
viewpoint =          'Earth'                               ; How view the output:
...
Observatory =        'keck'                                ; Set observer's location as a named observatory
                                                           ; Options: Any location within ASTROLIB's Observatory.pro, e.g.,     
                                                           ;          'CASLEO'            El Leoncito, Argentina 
                                                           ;          'McDonald'          McDonald Observatory, TX, USA
                                                           ;          'keck'              Mauna Kea, HI, USA
Inst_Pointing = [95.399886790, 38.049605983] / !radeg      ; [RA, DEC] of spectrometer pointing
Inst_Diameter = 60                                         ; Diameter of instrument aperture [arseconds]
Inst_LSF_wav  = 0.02                                       ; Gaussian sigma of instrumental line spread function at line of interest [angstroms]
...
Output_title           = 'Test'                            ; Title of the model's output FITS and Postscript files 
...
Time_range             = [0.,.1]                           ; When to start and stop releasing particles from the planet [End, Begin] days ago e.g., [0.0, 0.5]                     
;Time_range             = [.5]
                                                           ; A single element array represents a 1 sec 'pulse' of particles occuring [pulse_time] days ago
timestep               = 40.                               ; Time step in seconds from the RK4 integrator, use < = 50 s at Mercury 
...
;Upward_flux_at_exobase = 5.2e22                           ; Upward Flux accross the exobase in particles/s, integrated over 2 pi steradians, or for an instantaneous event, e.g. a meteor's total vaporization
;Upward_flux_at_exobase = 3.9e25 
;Upward_flux_at_exobase = 3.75e24                          ; dayside sputtering yeild of .001 atoms/ion precip of 10^10 /s /cm^2 (conservative value from Exner) 
Upward_flux_at_exobase = 2.27389e+024                      ; Szabo's (protons x surface area x proton yield) + (alphas x surface area x alpha yield)

;Seed                   = 2653553L                         ; A large scalar long integer to seed the random number generator, comment out if initializing randomly. 
Debug                  = 1                                 ; Set to 1 to output more detail at intermediate steps.        
exobase_height         = 0.d                               ; Exobase altitude above the surface, units of KM
outflow_v              = 0.                                ; Outflow velocity in km/s (non-zero for hydrothermal escape / comets)
if keyword_set(Loop_times_this_run) then Loop_times=Loop_times_this_run else $
Loop_times             = 50.                              ; How many loops the model should run, runs are stacked and averaged so this sets statistical noise  
FOV                    = 10.                               ; ARCSECONDS to a side. Field of View of output images (RA & Dec SKY COORDINATES)
...
sun_body_tickstep      = 0.5                               ; Axis tick step size in Body radii for the 'Sun-Body' viewings  
...
Plot_range             = 6.                                ; For 'Above ecliptic' viewing only. Defines the plate scale, the spatial distance of each axis to be plotted in BODY RADII
;===========================================KEYWORDS=======================================================================
...
Draw_Aperture          = 0                                 ; Draw the instrument aperture within which the emission line profile is measured
...
Above_Ecliptic         = 1                                 ; Writes additional output image viewed from above the ecliptic plane with "Plot_range" setting the field of view (longer run times)
restore_aloft          = 0                                 ; restore a loc array to start the integration
...
;                                            Speed_distribution, FOV, Output_Size_In_Pixels)
  ...
  ; Define some data cubes. Each loop will write an image to a layer of these. The they're averaged into final output images
    ...
    Velocity_Centers      = -50. + (0.1)/2. + 0.1*indgen(1000)                                                       ; velocity axis from -50 to +50 km/s on which to stack histograms
    Radial_v_Hist_Stack   = reform(dblarr(Velocity_Centers.length, loop_times), Velocity_Centers.length, loop_times) ; stack of radial velocity histograms in Rayleighs
    ...
    if keyword_set(Sun_Body) then Sun_Body_Cube_R  = reform(initialize_array, Output_Size_In_Pixels[0], Output_Size_In_Pixels[1], loop_times)
    if keyword_set(Sun_Body) then Sun_Body_Cube_CD = reform(initialize_array, Output_Size_In_Pixels[0], Output_Size_In_Pixels[1], loop_times)
  ...
  ; Get the cartesian state vector of the body at UTC in units of body radii / body radii per second
    ...
    if strmid(body, 0, 3) eq '100' then Body_radius =[1.,1.,1.] else $ ; comet's have radius 1.   
  ...
  ; Calculate the number of molecules/atoms that each test particle represents (test particles represent packets of atoms)
    ...
    atoms_per_packet = Upward_flux_at_exobase / release_rate                 ; (atoms/s)/(packets/s), the average packet (aka particle) content loc[4,*] MUST BE UNITY INITIALLY.
  ...
  ;=================================== IF THIS RUN IS A RE-START OF A PRIOR INTEGRATION ========================================================================================
  ...
  if keyword_set(restore_aloft_filename) then begin
    ...
    ; Adjust scattering rates by the illuminated fraction of the solar disc that each particle sees
      ...
      ; Write an output FITS file with the mean brightness [extension 0], column density [extension 1] and header info about the input parameters simulated
        ...
        output_display, loc, g, atoms_per_packet, Image_type, 30000, 2, reimpact_loc = reimpact_loc, label_phase = label_phase, Draw_Aperture = Draw_Aperture, Label_time = Label_time
  ...
  ;=================================== FOR A "FRESH" MODEL, START LOOPING THROUGH THE NUMBER OF MODEL RUNS TO CO-ADD ==============================================
  ...
  FOR loop_number = 0, loop_times - 1 do begin ;How many times the program should loop to reduce statistical noise
    ...
    loc = fltarr(10, Number_of_particles) ;Create a massive array of all particles
      ; loc[0,*] will be the x coords of all the particles in km [J2000, origin at whatever the body is orbiting]
      ; loc[1,*] will be the y coords of all the particles in km [J2000, origin at whatever the body is orbiting]
      ; loc[2,*] will be the z coords of all the particles in km [J2000, origin at whatever the body is orbiting]
      ; loc[3,*] will be the bodycentric distance of all the particles in km 
      ; loc[4,*] will be the *fractional* content of the packet (equals 1 initially unless weighted, 0 for packets hit w/o bouncing, ionization reduces this during integration] 
      ; loc[5,*] will be the x velocity of all the particles in km/s [J2000, origin at whatever the body is orbiting]
      ; loc[6,*] will be the v velocity of all the particles in km/s [J2000, origin at whatever the body is orbiting]
      ; loc[7,*] will be the z velocity of all the particles in km/s [J2000, origin at whatever the body is orbiting]
    ...
    Case n_elements(time_range) of
      1: loc[9,*] = replicate(mintime, Number_of_particles)                       ; A pulse of particles at point in time. Use for impactors
    ...
    ; Make starting locations and velocities for particles (J2000, origin is at whatever Body is orbiting, solar system barycenter, planet barycenter etc.)
      cspice_spkpos, Body, ephemeris_time, 'J2000', 'LT', viewpoint, Body_Observer_vector, Obs_Body_Ltime ; light time needed to do this right
    ...
    ; Make starting locations for particles
      loc[0:2,*] = loc[0:2,*]*(Body_radius[0] + exobase_height) ; Scale the "release state" vectors from body radii coordinates to the exobase (km)        
      if total(loc[4,*]) eq 0. then loc[4,*] = 1.0              ; Packet content is initially full ***UNLESS weighting is previously specified in RELEASE_STATE***
      loc[5,*] = loc[5,*]*speed + loc[5,*]*outflow_v            ; Give them initial velocities in x; units are km/s, speed array is in km/s
      loc[6,*] = loc[6,*]*speed + loc[6,*]*outflow_v            ; Give them initial velocities in y; units are km/s, speed array is in km/s
      loc[7,*] = loc[7,*]*speed + loc[7,*]*outflow_v            ; Give them initial velocities in z; units are km/s, speed array is in km/s
      loc[8,*] = 0.0D                                           ; Particles start out at 0 seconds in age, they haven't aged yet   

    
    ; -----------------For MOONS, model's origin is the PLANET'S BARYCENTER------------------------
    ; -----------------For COMETS & PLANETS, model's origin is the SOLAR SYSTEM BARYCENTER---------------------
    ;  See generic_model V1 vs V2 codes for structural differences in Barycenter origin vs body-centered origin
      cspice_bods2c, body, bodyID_code, found
      if (STRPOS(string(bodyID_code), '99') eq -1) then $                                ; if Body is not a Planet...
        Parent_ID = strcompress(strmid(strcompress(bodyID_code, /remove_all),0,1)) else $; then assign a Planetary Barycenter as the object it's orbiting (moons)
        Parent_ID = '0'                                                                  ; if it is a planet, then origin is the solar system barycenter    
        if (strmid(body, 0, 3) eq '100') then Parent_ID = '0'                            ; if it is a comet, then origin is the solar system barycenter   
      cspice_bodc2n, long(Parent_ID), Parent_name, origin_found
      if origin_found then print, 'Model''s origin defined as: ', Parent_name else print, 'Parent body not found!' 
        
    ; **** Add the body's instantaneous orbital position and velocity at the time of the particles release ****
    
    ; for this random distribution of packet launch times, get the corresponding state vector of the body
    ; Account for the Earth-Body light time: we're observing the body as it was that light time ago.
      cspice_spkezr, body, ephemeris_time - Obs_Body_Ltime - reform(loc[9,*]), 'J2000', 'None', Parent_ID, state_at_release, junk_light_time 

      ; state is [x,y,z,vx,vy,vz] with respect to the object we're orbiting
      ; Add the body's J2000 orbital position (relative to origin aka planet center) to the inital x,y,z packet positions (relative to an origin of the body, J2000) 
        loc[0:2,*] = loc[0:2,*] + state_at_release[0:2,*]
      ; To the inital packet velocities, add orbital velocity components at the release time: vx,vy and vz
        loc[5:7,*] = loc[5:7,*] + state_at_release[3:5,*] ;FUTURE WORK should consider using the surface location velocities, not just the velocity of the planet.
    
    
    ;-------------Debug: check that Loc coord RA and Dec matches the bodies RA and Dec-------
    
;    cspice_spkpos, Parent_ID, ephemeris_time, 'J2000', 'LT', viewpoint, Origin_Observer_vector, light_time
;    cspice_recrad, (Origin_Observer_vector), Origin_distance_WRT_obs, Origin_RA_WRT_obs, Origin_dec_WRT_obs
;    N_particles = Number_of_particles
;    cspice_recrad, loc[0:2,*] + rebin(Origin_Observer_vector, 3, N_particles), $
;      Loc_distance_WRT_obs, Loc_RA_WRT_obs, Loc_dec_WRT_obs
;    
;    print, 206265.* (174.72353781250669/(180.d/!dpi) - mean(Loc_RA_WRT_obs))  
;    print, 206265.* (1.3266429700126587/(180.d/!dpi) - mean(Loc_Dec_WRT_obs))  
;    print, 'you must zero this out!'
;    stop
    ;--------------------end Debug-----------------------------------
    
    
    ; Convert the initial release flux into planetographic lon and lat ---> this section works once for a specific application, but needs testing.
    ...
    if strmid(body, 0, 3) eq '100' then release_map = fltarr(360,180) else begin
    ...
    ; Compute scattering rates for each packet in the exosphere
      ...
      ; Adjust scattering rates by the illuminated fraction of the solar disc that each particle sees
      ; Illumination.pro needs J2000 coords in the body centered frame   
      cspice_spkPOS, body, ephemeris_time - Obs_Body_Ltime, 'J2000', 'None', Parent_ID, state_at_end, junk_light_time 
        airborn_packets = where(loc[4,*] ne 0., number_airborn, /NULL)
        if number_airborn gt 0 then begin
          penum = illumination(fltarr( number_airborn ), loc[0,airborn_packets] - state_at_end[0], $ ; Subtract origin to get body center J2000 coords
                                                         loc[1,airborn_packets] - state_at_end[1], $
                                                         loc[2,airborn_packets] - state_at_end[2]) 
          g[airborn_packets] = g[airborn_packets] * penum                                            ; Only sunlit packets emit  
        endif
    ...
    ; Build the images in both Rayleighs and Column Density by co-adding 
      ...
      restore, strcompress(directory+'Model_Radial_v_Hist'+string(loop_number)+'.sav')
      ...
      if keyword_set(Sun_Body) then begin
        restore, strcompress(directory+'Sun_Body_Image_R'+string(loop_number)+'.sav')
        restore, strcompress(directory+'Sun_Body_Image_CD'+string(loop_number)+'.sav')
        Sun_Body_Cube_R[*,*,Loop_number]  = Sun_Body_Image_R
        Sun_Body_Cube_CD[*,*,Loop_number] = Sun_Body_Image_CD
      endif  
      ...
      Radial_v_Hist_Stack[*,Loop_number] = interpol(Radial_v_Hist, centers, Velocity_Centers)
    
    ; Save the big array for steady state release over some durations only  
      if N_elements(Time_range) gt 1 then save, loc, filename = strcompress(directory + Output_title + '_Loc_Array_'+string(loop_number)+'.sav')  
    
    ; Write an array of exosphere's particle content in the body fixed frame:
      if strmid(body, 0, 3) ne '100' then begin                                       ; skip for small bodies without a body fixed frame
  ...
  ; Average (mean) the brightness and column density over the number of model runs in the above loop 
    ...
    if keyword_set(Sun_Body) then Sun_Body_Image_R  = mean(Sun_Body_Cube_R,  dimension = 3)
    if keyword_set(Sun_Body) then Sun_Body_Image_CD = mean(Sun_Body_Cube_CD, dimension = 3)
    Radial_v_Hist_avg    = mean(Radial_v_Hist_Stack, dimension = 2)
  ...
  ; Write an output FITS file with the mean brightness [extension 0], column density [extension 1] and header info about the input parameters simulated
    ...
    if keyword_set(Sun_Body) then mwrfits, Sun_Body_Image_R, strcompress(directory+Output_title+'_Sun_Body.fit'), header, /create, /Silent
    if keyword_set(Sun_Body) then mwrfits, Sun_Body_Image_CD, strcompress(directory+Output_title+'_Sun_Body.fit'), /Silent
  ...
  ; Write an output save file with the velocity centers and mean velocity line profile
    save, Velocity_Centers, Radial_v_Hist_avg, filename=strcompress(directory+Output_title+'Radial_v_Hist.sav')
  ...
  ; Plot the final loop-averaged results 
    ...
    output_display, loc, g, atoms_per_packet, Image_type, loop_number, 2, reimpact_loc = reimpact_loc, label_phase = label_phase, Draw_Aperture = Draw_Aperture, Label_time = Label_time
