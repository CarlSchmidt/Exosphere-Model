...
;===========================================INPUTS=======================================================================
Directory          = 'C:\IDL\Generic Model V4\read_write\' ; Directory where all other files are read from and written to
...
;UTC                = '2013 November 29 14:25'             ; Juno-Europa Flyby (Io in Transit, Europa wester elongation'
...
;UTC                = '2020-Jul-15 11:43:56'               ; NEOWISE Jul 15 EXPRES
UTC                = '2020-Jul-15 03:42'                   ; NEOWISE Jul 15 IoIO
...
Surface_distribution = 'Global'                            ; Set the surface distribution the particles will be released with
                                                           ...
                                                           ;          'Dust'              Seeds from previous run saved loc arrays
                                                           ;          'From_Map:'         TBD
...
Observatory =        'lowell'                              ; Set observer's location as a named observatory
...
Inst_Pointing = [116.060582273d, 46.786439197d] / !radeg     ; [RA, DEC] of spectrometer pointing
;Inst_Pointing = [117.574025701, 47.056177505] / !radeg    ; EXPRES Jul 15
Inst_Diameter = 2                                          ; Diameter of spectrometer aperture [arseconds]
Inst_LSF_wav  = 0.02                                       ; Gaussian sigma of instrumental line spread function at wavelength of interest [angstroms]
...
Output_title           = 'V5_Merge_Test'                   ; Title of the model's output FITS and Postscript files 
...
Time_range             = [0., 0.001]                         ; When to start and stop releasing particles from the planet [End, Begin] days ago e.g., [0.0, 0.5]                     
...
Upward_flux_at_exobase = 1.e25                             ; Upward Flux across the exobase in particles per second, integrated over 2 pi steradians, or for an intantaneous event, e.g. a meteor's total vapourization
...
sun_body_tickstep      = 50000                             ; Axis tick step size in Body radii for the 'Sun-Body' viewings  
...
Plot_range             = 300.                              ; For 'Above ecliptic' viewing only. Defines the plate scale, the spatial distance of each axis to be plotted in BODY RADII
;===========================================KEYWORDS=======================================================================
...
; (output)_Column_Density.eps         = Postscript plot of the column density averaged over all integration runs
; (output)_Emission.eps               = Postscript plot of the brightness averaged over all integration runs
; (output)_Temperature.eps            = Postscript plot of the spectral temperature averaged over all integration runs
;*******************************************************************************************************
...
;                                            Speed_distribution, FOV, Output_Size_In_Pixels)
  ...
  ; Define some data cubes. Each loop will write an image to a layer of these. The they're averaged into final output images
    ...
    Model_Image_RV        = fltarr(Output_Size_In_Pixels[0], Output_Size_In_Pixels[1], 2000)                         ; radial velocity histogram image
    Velocity_Centers      = -100. + (0.1)/2. + 0.1*indgen(2000)                                                      ; velocity axis from -100 to +100 km/s on which to stack aperture histograms
    Radial_v_Hist_Stack   = reform(dblarr(Velocity_Centers.length, loop_times), Velocity_Centers.length, loop_times) ; stack of aperture radial velocity histograms in (Rayleighs but BROKEN)
  ...
  ; use a fixed photoionization lifetime. Skip the Irradiance.pro calculation of time-dependent lifetimes
    Case test_particle of ; Force photoionization lifetime in seconds (Huebner & Mukherjee, 2015) Above calculation Irradiance.pro yields longer lifetimes!
      ...
      'SO2': ionizelife  = !values.F_NaN  ; to do 
    endcase
  ...
  FOR loop_number = 0, loop_times - 1 do begin ;How many times the program should loop to reduce statistical noise
    ...
    loc = fltarr(10, Number_of_particles) ;Create a massive array of all particles
      ...
      ; loc(4,*) will be the fractional content of the original packet (after they hit = 0, ionization (when included) should give a fraction of 1) 
...
;    stop
    ...
    ; Write the images for that loop
      output_display, loc, g, atoms_per_packet, Image_type, Model_Image_RV, loop_number, 1, reimpact_loc = reimpact_loc
  ...
  ; Plot the final loop-averaged results
    ...
    output_display, loc, g, atoms_per_packet, Image_type, Model_Image_RV, loop_number, 2, reimpact_loc = reimpact_loc, label_phase = label_phase, Draw_Aperture = Draw_Aperture, Label_time = Label_time
