...
;
; CALLING SEQUENCE:
;   Result = Bin_2_SurfDens(V1, V2, W, Binsize)
; INPUTS:
...
; 
;-
function Bin_2_SurfDens, im1, im2, ww,$
  ...
  ; Determine the surface area per bin using Girard's theorem
  ...
  for i = -90., 90.-lat_bin, lat_bin do begin
    ...
    lat0 = i
    lat1 = i + lat_bin
    quadrangle_area[i] = body_radius[0]^2 * abs( sin(lat0/!radeg) - sin(lat1/!radeg) ) * abs(lon0/!radeg - lon1/!radeg) / lat_bin
  endfor
  
  surface_dens = rebin( transpose(quadrangle_area), 360. / lon_bin)
